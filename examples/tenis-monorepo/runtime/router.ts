...
import {
  ...
  DocumentNotFoundForUpdateError,
  FirestoreRepository,
...
function parsePathBody(body: { collection?: string; id?: string; path?: string[] }): string[] | null {
  if (Array.isArray(body.path) && body.path.length >= 2 && body.path.length % 2 === 0) {
  ...
  try {
    ...
    if (method === 'POST' && path === '/api/create') {
      return handleGenericCrud(request, async () => {
        ...
        assertPathAllowed(pathSegments);
        await firestoreRepository.createDocumentByPath(pathSegments, body.data);
        return jsonResponse(201, { success: true, data: { id: pathSegments.at(-1) } });
      });
    }

    if (method === 'POST' && path === '/api/upsert') {
      return handleGenericCrud(request, async () => {
        const body = request.body as {
          collection?: string;
          id?: string;
        ...
        assertPathAllowed(pathSegments);
        await firestoreRepository.upsertDocumentByPath(pathSegments, body.data);
        return jsonResponse(200, { success: true, data: { id: pathSegments.at(-1) } });
      });
    }
    ...
    if (
      ...
      error instanceof CollectionNotAllowedError ||
      error instanceof DocumentAlreadyExistsError ||
      error instanceof DocumentNotFoundForUpdateError
    ) {
      const code =
        error instanceof DocumentAlreadyExistsError ||
        error instanceof DocumentNotFoundForUpdateError
      ...
      return jsonResponse(error.status, { success: false, message: error.message, code });
    }
