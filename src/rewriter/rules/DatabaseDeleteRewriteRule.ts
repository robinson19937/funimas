...
export class DatabaseDeleteRewriteRule implements RewriteRule {
  readonly id = 'database-delete-rewrite';
  readonly name = 'DatabaseDeleteRewriteRule';

  canApply(operation: SemanticOperation): boolean {
    return (
      operation.type === 'DATABASE_DELETE' &&
      operation.metadata.callee === 'deleteDoc'
    );
  }
  ...
  async apply(context: RewriteContext, operation: SemanticOperation): Promise<RewriteApplication | null> {
    ...
    if (!callExpression || callExpression.getExpression().getText() !== 'deleteDoc') {
      return null;
    }
    ...
    return {
      ...
      ruleId: this.id,
      ruleName: this.name,
      reason: TransformationReason.forOperation(operation.type, 'deleteDoc'),
      benefit: TransformationBenefit.forOperation(operation.type, 'deleteDoc'),
      riskLevel: 'LOW',
